...
trait SpiderFormClassHtmlBuilderTrait
{
    protected
        $imploded_output = [],
        $html_output = [],
    ...
    public function buildHtmlOutput(): void
    {
        ...
        $builderClassName = $this->getSpiderFormBuilderClassName();
     ...
     * 13:38
     */
    private function getSpiderFormBuilderClassName(): string
    {
        if ($this->hasUserDefinedSpiderFormBuilderObjects()) {
            $namespace = $this->getUserDefinedSpiderFormBuilderNamespace();
        } else {
            $namespace = $this->namespace;
        }

        $class = $namespace . '\SpiderFormBuilder\\' . $this->build_format . 'Builder';
        if (!class_exists($class)) {
            $class = $namespace . '\SpiderFormBuilder\GenericBuilder';
        }
        return $class;
    }
     ...
     * 11:55
     */
    private function hasUserDefinedSpiderFormBuilderObjects(): bool
    {
     ...
     * 11:56
     */
    private function getUserDefinedSpiderFormBuilderNamespace()
    {
