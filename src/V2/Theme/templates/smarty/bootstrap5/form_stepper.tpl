...
<form {$form.attributes|attributes nofilter}>
    ...
    <div class="stepper stepper-{$stepper_options.layout|default:'horizontal'}" data-stepper="{$form.name}">
        {* Stepper Navigation *}
        <div class="stepper-nav {if $stepper_options.layout == 'vertical'}flex-column{else}d-flex justify-content-between{/if} mb-4">
            {assign var="step_index" value=0}
            {foreach $inputs as $input}
                {if $input.is_section && $input.section}
                    <div class="stepper-nav-item {if $step_index == $stepper_options.startIndex|default:0}active{/if}" data-stepper-nav="{$step_index}">
                        ...
                        <div class="stepper-nav-label">
                            ...
                            {if $input.section.description}
                                <div class="stepper-nav-desc text-muted small">{$input.section.description nofilter}</div>
                            {/if}
                        </div>
                    </div>
        ...
        <div class="stepper-content">
            ...
            {foreach $inputs as $input}
                {if $input.is_section}
                    ...
                    {if $input.section}
                        <div class="stepper-step {if $step_index == $stepper_options.startIndex|default:0}active{/if}" data-stepper-step="{$step_index}" style="display: {if $step_index == $stepper_options.startIndex|default:0}block{else}none{/if};">
                            <div class="mb-4">
                                <h3 class="fw-bold mb-2">{$input.section.title}</h3>
                                {if $input.section.description}
                                    <p class="text-muted">{$input.section.description nofilter}</p>
                                {/if}
